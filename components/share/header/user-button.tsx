...
async function UserButton() {
  ...
  if (!session) {
    return (
      <Button
        asChild
        className="bg-brand dark:text-white text-black font-bold hover:bg-slate-200/60 hover:text-brand"
      >
        <Link href="/sign-in">
          <UserCircle /> Sign in
        </Link>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="flex justify-center items-center">
            <Button
              variant="ghost"
              className="bg-gray-200 text-brand rounded-full w-8 h-8 relative ml-2 flex justify-center items-center"
            >
              {firstLetterOfName}
            </Button>
          </div>
